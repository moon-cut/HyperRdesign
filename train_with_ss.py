...
train_dataset = RNADataset(
    ...
    data_config.train_ss_data_dir,# added by liu 0929
    data_config.train_data_path,
)
valid_dataset = RNADataset(
    data_config.valid_npy_data_dir,
    data_config.valid_ss_data_dir,# added by liu 0929
...
for epoch in range(train_config.epoch):
    ...
    for batch in train_pbar:
        X, S, mask, lengths, names ,ss_info_list = batch
        X = perturb_coordinates(X, sigma=0.2)
        X = X.to(config.device)
        S = S.to(config.device)
        mask = mask.to(config.device)
        logits, S= model(X, S, mask,ss_info_list)
    ...
    with torch.no_grad():
        ...
        if valid_recovery > best_valid_recovery:
            ...
            torch.save(model.state_dict(), os.path.join(train_config.output_dir, 'HyperRdesign.pt'))
